...
export const createAddtofav = (userId,id) => {
    ...
    return (dispatch) => {
        ...
        /// post request
        fetch(`https://api.cvvlogs.com/cv-tube/api.v.2/recruiter/web/add_to_fav.php`, {
            method: 'POST',
...
export const createAddtofavsearch = (userId,id,keyword,skill,state,ind,exp) => {
    console.log(userId,id)
    return (dispatch) => {
        dispatch({
            type: "ADDTOFAV_RESET",
...
export const getAllFavs = (userId) => {
    return (dispatch) => {
        /// get request
        fetch(`${config['baseUrl']}/web/all_favs.php`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json',"auth_id":`${userId}` },
            // body: JSON.stringify({
            //     "data":{
