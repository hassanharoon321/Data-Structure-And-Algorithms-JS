...
const MStripe = (props) => {
  ...
  const onPost = () => {
    ...
    /// post request
    fetch("https://api.cvvlogs.com/cv-tube/api.v.2/recruiter/web/paid_for.php", {
      method: 'POST',
  ...
  const onToken = (token, addresses) => {
    ...
    /// post request
    fetch("https://api.cvvlogs.com/cv-tube/api.v.2/recruiter/web/package_payment_confirmation.php", {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', "auth_id": `${localStorage.getItem("auth_id1")}` },
      body: JSON.stringify({
